...
resource "tfe_variable" "vault_backed_aws_additional_role_arns" {
  for_each = toset(local.additional_aws_account_ids)

  key          = "TFC_VAULT_BACKED_AWS_RUN_ROLE_ARN_${each.value}"
  value        = "arn:aws:iam::${each.value}:role/${var.aws_iam_role_name}"
  category     = "env"
  description  = "ARN of the AWS role to assume for account ${each.value}"
  workspace_id = var.workspace_id
}
...
resource "tfe_variable" "vault_backed_aws_auth_additional" {
  ...
  value        = "true"
  category     = "env"
  description  = "Enable Vault-backed AWS authentication for account ${each.value}"
  workspace_id = var.workspace_id
}
...
resource "tfe_variable" "vault_backed_aws_run_vault_role_additional" {
  ...
  value        = vault_aws_secret_backend_role.vault_aws_role.name
  category     = "env"
  description  = "Vault role for AWS authentication for account ${each.value}"
  workspace_id = var.workspace_id
}
