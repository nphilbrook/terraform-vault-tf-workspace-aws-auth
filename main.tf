resource "vault_jwt_auth_backend_role" "vault_jwt_tfe_workspace_role" {
  namespace = var.vault_namespace_path
  backend   = var.jwt_auth_path
  role_name = "tfe-workspace-${var.workspace_name}"

  token_policies = ["default", vault_policy.tfe_workspace_aws_policy.name]
...
resource "vault_aws_secret_backend_role" "vault_aws_role" {
  ...
  name            = "tfe-workspace-${var.workspace_name}"
  credential_type = "assumed_role"
  role_arns       = [for account_id in var.aws_account_ids : "arn:aws:iam::${account_id}:role/${var.aws_iam_role_name}"]
}

resource "vault_policy" "tfe_workspace_aws_policy" {
  namespace = var.vault_namespace_path
  name      = "tfe-workspace-${var.workspace_name}"
  policy    = data.vault_policy_document.tfe_workspace_aws_policy.hcl
}

data "vault_policy_document" "tfe_workspace_aws_policy" {
  rule {
